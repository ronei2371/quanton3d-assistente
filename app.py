...
import os, re, uuid, base64, hashlib, random, json
...
ADMIN_TOKEN = os.getenv("ADMIN_TOKEN", "").strip()  # opcional, pode ficar vazio
...
def admin_blocked_list():
    ...
    return jsonify(sorted(load_blocked())), 200
...
def admin_block():
    ...
    if not phone:
        return ("phone obrigatório", 400)
    s = load_blocked(); s.add(phone); save_blocked(s)
    return jsonify({"ok": True, "blocked": sorted(s)}), 200
...
def api_chat():
    ...
    try:
        ...
        try:
            ...
            # se blocked.json não existir ainda, ignora
            pass

        # Fast-path: amarelamento após cura (resposta com tempos numéricos antes do GPT)
        pl = (problem or "").lower()
        if any(k in pl for k in ("amarel", "amarela", "amarelamento")):
            resposta = (
                ...
                "Checklist: lavar bem; SECAR totalmente (sem IPA preso); evitar calor na câmara."
            )
            return jsonify({
                "ok": True,
                "answer": resposta,
